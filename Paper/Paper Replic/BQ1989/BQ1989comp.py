...
# --- FEVD COMPARISON EXTENSION ---

# 1. Load the benchmark FEVD data
# Loads all sheets into a dictionary to bypass exact name matching
data_fevd_dict = pd.read_excel("BQ1989_FEVD.xlsx")

# Extract sheets for Shock 1 and Shock 2
VD_excel = [data_fevd_dict[['yy','uy']],data_fevd_dict[['yu','uu']]]

# 2. Extract macroshock FEVD points 
VD1 = modelo.VarianceDecomp(shock=1, show=False)
VD2 = modelo.VarianceDecomp(shock=2, show=False)
VD_macro = [VD1, VD2]

# 3. Plotting loop for FEVD Comparison (Lines only)
for j in range(modelo.n_vars):
    ...
    for i in range(modelo.n_vars):
        ...
        # Macroshock point estimate (Smooth Solid Line)
        macro_point = VD_macro[j]['point'][var_name]
        ax.plot(horizon, macro_point, color="darkred", linestyle="-", linewidth=2.5, label="macroshock")
        
        # BQ1989 Excel benchmark estimate (Dotted Line with Stars)
        # Matches by column name if it exists, otherwise falls back to positional index
        ...
        # Check if the Excel data needs to be scaled down from percentage to decimal (0-1)
