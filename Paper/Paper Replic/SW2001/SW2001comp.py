...
for j in range(modelo.n_vars):
    ...
    for i in range(modelo.n_vars):
        ...
        # SW2001 Excel benchmark estimate (Dotted Line)
        # Assumes Excel columns either perfectly match the variable names OR maps positionally 
        if var_name in VD_excel[j].columns:
            excel_point = VD_excel[j][var_name]
        else:
            # Shift index by 1 if the first column happens to be the step/horizon index
        ...
        # Adjust y-limits based on whether data is decimal (0-1) or percentage (0-100)
